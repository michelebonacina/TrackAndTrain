...
exports.authenticationMiddleware = function (request, response, next)
{
    ...
    {
        // password is wrong
        return response.status(401).send({ errors: [{ title: 'Unauthorized', details: 'Authentication token is not valid' }] });
    }
 ...
 * @returns decoded token, null if there're some errors
 */
parseToken = function (token)
{
    ...
    try
    {
        decodedToken = jwt.verify(token.split(' ')[1], config.SECRET);
    }
    catch (error)
    {
